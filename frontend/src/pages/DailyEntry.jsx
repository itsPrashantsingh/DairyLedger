...
export default function DailyEntry() {
  ...
  async function saveAll() {
    ...
    if (errors.length) {
      ...
      setToast({ message: `✓ Saved ${delivered} customer deliveries`, type: 'success' })
      setDirty(false)
      loadData()
    }
  }
    ...
    <div className="pb-36">
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500">Milk given to each customer</p>
        </div>
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] backdrop-blur md:bottom-0 md:left-56">
        ...
        >
          {saving ? 'Saving...' : dirty ? '💾 Save Deliveries' : '💾 Saved'}
        </button>
      </div>
    </div>
  )
